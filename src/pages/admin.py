...
def show_page(data_manager: DataManager):
    ...
    try:
        ...
        st.error("Admin password not configured in secrets.toml. Please add `[admin_panel]` section with `admin_password`.")
        return
...
def _show_episode_results_tab(dm: DataManager):
    ...
    with st.form(f"results_week_{result_week_key}"):
        ...
        with col1:
            sb = st.selectbox("⭐ Actual Star Baker:", baker_options, index=baker_options.index(existing_results.get('star_baker')) if existing_results.get('star_baker') in baker_options else 0)
            tw = st.selectbox("🏆 Technical Winner:", baker_options, index=baker_options.index(existing_results.get('technical_winner')) if existing_results.get('technical_winner') in baker_options else 0)
        with col2:
            hh = st.checkbox("🤝 Was a Handshake given?", value=existing_results.get('handshake_given', False))
            eb = st.selectbox("😢 Baker Eliminated:", baker_options, index=baker_options.index(existing_results.get('eliminated_baker')) if existing_results.get('eliminated_baker') in baker_options else 0)

        if st.form_submit_button("Save Episode Results"):
            results_data = {'star_baker': sb, 'technical_winner': tw, 'handshake_given': hh, 'eliminated_baker': eb, 'updated_at': datetime.now().isoformat()}
            ...
            df_data = [{'Player': data['users'].get(uid,{}).get('name'), 'Weekly': s['weekly_points'], 'Foresight': s['foresight_points'], 'Total': s['total_points']} for uid, s in updated_scores.items()]
            leaderboard_df = pd.DataFrame(df_data).sort_values('Total', ascending=False)
            leaderboard_df.index = range(1, len(leaderboard_df) + 1)
            send_commissioner_update_email(WEEK_DATES.get(result_week_key), results_data, leaderboard_df)
...
def _show_manage_players_tab(dm: DataManager):
    ...
    if player_to_edit:
        ...
        with st.expander("🗑️ Remove Player (Permanent)"):
            st.warning(f"**Warning:** This will permanently delete **{user_info.get('name')}** and all of their picks.")
...
def _show_final_scoring_tab(dm: DataManager):
    ...
    st.info("Use this tool **only after the season finale** to calculate final Foresight Points.")
    data = dm.get_data()
    baker_options = data.get('bakers', [])
        ...
        if st.form_submit_button("CALCULATE & SAVE FINAL SCORES"):
            if final_winner and len(final_finalists) == 2:
                if final_winner not in final_finalists:
                    ...
                    player_names = {uid: u.get('name', 'Unknown') for uid, u in data.get('users', {}).items()}
