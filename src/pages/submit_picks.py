...
def show_page(data_manager: DataManager):
    ...
    player_options = {
        uid: uinfo.get("name", f"Unknown User ({uid})")
    ...
    selection = st.selectbox(
        ...
        if x is None
        else ("Create New Profile" if x == "➕ New Player" else player_options.get(x, "Unknown")),
    )
            ...
            if st.form_submit_button("Create Profile"):
                if new_name and new_email and "@" in new_email and "." in new_email:
                    if is_email_allowed(new_email):
                        email_exists = any(
                            user.get("email", "").lower() == new_email.lower()
                            for user in data.get("users", {}).values()
                        )
                        if email_exists:
                            st.error(
                                "This email address is already registered. Please select your profile from the dropdown menu."
                            )
                        else:
                        ...
                        st.error(
                            "This email address has not been approved for the league. Please contact the commissioner to be added."
                        )
                else:
    ...
    if st.session_state.selected_user:
        ...
        available_weeks = [
            k for k, v in REVEAL_DATES_UTC.items() if now_utc < v
        ]
        ...
        with col1:
            ...
            sb = st.selectbox(
                ...
                key=f"sb_{user_id}_{selected_week}",
            )
            tw = st.selectbox(
                "🏆 Technical Winner:",
                bakers,
                ...
                key=f"tw_{user_id}_{selected_week}",
            )
            eb = st.selectbox(
                "😢 Sent Home:",
                bakers,
                ...
                key=f"elim_{user_id}_{selected_week}",
            )
            hh = st.checkbox(
                "🤝 Hollywood Handshake?",
                value=existing_picks.get("handshake_prediction", False),
                key=f"hh_{user_id}_{selected_week}",
            )
        with col2:
            ...
            sw = st.selectbox(
                ...
                key=f"sw_{user_id}_{selected_week}",
            )
            f1 = st.selectbox(
                "🥈 Finalist A:",
                bakers,
                index=bakers.index(existing_picks.get("finalist_1", bakers[1]))
                if "finalist_1" in existing_picks
                and existing_picks["finalist_1"] in bakers
                and len(bakers) > 1
                else 1,
                key=f"f1_{user_id}_{selected_week}",
            )
            f2 = st.selectbox(
                "🥈 Finalist B:",
                bakers,
                index=bakers.index(existing_picks.get("finalist_2", bakers[2]))
                if "finalist_2" in existing_picks
                and existing_picks["finalist_2"] in bakers
                and len(bakers) > 2
                else 2,
                key=f"f2_{user_id}_{selected_week}",
            )
