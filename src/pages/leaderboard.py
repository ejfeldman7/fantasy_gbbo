...
def show_page(data_manager: DataManager):
    ...
    for user_id, user_info in data.get("users", {}).items():
        user_score = scores.get(
            user_id, {"weekly_points": 0, "foresight_points": 0, "total_points": 0}
        )
    ...
    with st.expander("📋 View All Picks History"):
        if data.get("picks"):
            ...
            all_weeks = sorted(
                set(w for picks in data["picks"].values() for w in picks.keys()), key=int
            )
                    ...
                    with st.expander(f"{display_name} Predictions"):
                        ...
                        for user_id, user_picks in data["picks"].items():
                            if week_key in user_picks:
                                ...
                                player_name = data["users"].get(user_id, {}).get(
                                ...
                                week_picks_data.append(
                                    {
                                        ...
                                        "Handshake": "✓"
                                        if picks.get("handshake_prediction")
                                        else "✗",
